...
public class YDXSeekBar extends RelativeLayout {
    ...
    public YDXSeekBar(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initAttrs(attrs);
        initView();
    }

    public YDXSeekBar(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(AttributeSet attributeSet) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attributeSet, R.styleable.YDXSeekBar);
        isNightMode = typedArray.getBoolean(R.styleable.YDXSeekBar_seekbarNightmode, false);
        thumbDrawable = typedArray.getDrawable(R.styleable.YDXSeekBar_android_thumb);
        progressDrawable = (LayerDrawable) typedArray.getDrawable(R.styleable.YDXSeekBar_android_progressDrawable);
        progressHeight = (int) typedArray.getDimension(R.styleable.YDXSeekBar_progressHight,20);
        thumbSize = (int) typedArray.getDimension(R.styleable.YDXSeekBar_thumbSize,20);
        anchorSize = (int) typedArray.getDimension(R.styleable.YDXSeekBar_anchorSize,20);
        showProgessText = typedArray.getBoolean(R.styleable.YDXSeekBar_showProgresstext, false);
        viewWitdh = (int) typedArray.getDimension(R.styleable.YDXSeekBar_android_layout_width,0);

        leftDrawable = typedArray.getDrawable(R.styleable.YDXSeekBar_progressDrawableLeft);
        rightDrawable = typedArray.getDrawable(R.styleable.YDXSeekBar_progressDrawableRight);

        leftDrawableNight = typedArray.getDrawable(R.styleable.YDXSeekBar_progressDrawableLeftNight);
        rightDrawableNight = typedArray.getDrawable(R.styleable.YDXSeekBar_progressDrawableRightNight);

        leftImageSize = (int) typedArray.getDimension(R.styleable.YDXSeekBar_progressDrawableLeftSize,20);
        rightImageSize = (int) typedArray.getDimension(R.styleable.YDXSeekBar_progressDrawableRightSize,20);

        showAnchor = typedArray.getBoolean(R.styleable.YDXSeekBar_showAnchor, false);
        anchorDrawableDaymode = typedArray.getDrawable(R.styleable.YDXSeekBar_anchorDrawable);
        anchorDrawableNightmode = typedArray.getDrawable(R.styleable.YDXSeekBar_anchorNightDrawable);
    }

    private void initView() {
        LayoutInflater.from(getContext()).inflate(R.layout.ydx_seekbar, this, true);
    ...
    DayNightImageView mLeftImage;
    DayNightImageView mRightImage;
...
//        drawScal(canvas,10);
    ...
    private void toggleNightView() {
        mLeftImage.setNightMode(isNightMode);
        mRightImage.setNightMode(isNightMode);
        if (isNightMode) {
            if (showAnchor) {
                anchorView.setBackground(anchorDrawableNightmode);
            }
        }else {
            if (showAnchor) {
                anchorView.setBackground(anchorDrawableDaymode);
            }
        }
    }
    ...
    public void setAnchorProgress(int progress) {
        ...
        RelativeLayout.LayoutParams layoutParams = (LayoutParams) anchorView.getLayoutParams();
        int maxLeft = backView.getWidth()-anchorView.getWidth();
...
//        backView.setLayoutParams(layoutParams3);
    ...
    private YDXSeekBarListener mListener;

    public void setOnSeekBarChangeListener(YDXSeekBarListener listener) {
        mListener = listener;
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
    ...
    private void setLeftRightDrawable() {
        if (leftDrawable != null) {
            ...
            mLeftImage.setDayImage(leftDrawable);
            if (leftDrawableNight!=null) {
                mLeftImage.setNightImage(leftDrawableNight);
            }
        ...
        if (rightDrawable != null) {
            ...
            mRightImage.setDayImage(rightDrawable);
            if (rightDrawableNight != null) {
                mRightImage.setNightImage(rightDrawableNight);
            }
    ...
    public interface YDXSeekBarListener{
